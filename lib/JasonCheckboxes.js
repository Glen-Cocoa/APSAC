...
var listStates = function(){
	for(var i = 0; i < states.length; i++){ 
		states_li = $("<div id="+states[i]+"><input class='clearTarget' type='checkbox' name="+states[i]+" rel='States' value="+states[i]+" id="+states[i]+"><label for="+states[i]+">"+states[i]+"</label><br></div>")
...
var listTypes = function(){
	for(var i = 0; i < types.length; i++){
		types_li = $("<div id="+types[i]+"><input class='clearTarget' type='checkbox' value="+types[i]+" rel='Types' id="+types[i]+"><label for="+types[i]+">"+types[i]+"</label><br></div>")
...
var listAcademicDepartments = function(){
	for(var i = 0; i < academic_departments.length; i++){
		academic_departments_li = $("<div id="+academic_departments[i]+"><input class='clearTarget' type='checkbox' value="+academic_departments[i]+" rel='Academic Departments' id="+academic_departments[i]+"><label for="+academic_departments[i]+">"+academic_departments[i]+"</label><br></div>")
...
var listDegreeTypes = function(){
	for(var i = 0; i < degree_types.length; i++){
		degree_types_li = $("<div id="+degree_types[i]+"><input class='clearTarget' type='checkbox' value="+degree_types[i]+" rel='Degree Types' id="+degree_types[i]+"><label for="+degree_types[i]+">"+degree_types[i]+"</label><br></div>")
...
var listLevelsOfStudy = function(){
	for(var i = 0; i < levels_of_study.length; i++){
		levels_of_study_li = $("<div id="+levels_of_study[i]+"><input class='clearTarget' type='checkbox' value="+levels_of_study[i]+" rel='Levels of Study Available' id="+levels_of_study[i]+"><label for="+levels_of_study[i]+">"+levels_of_study[i]+"</label><br></div>")
...
var listStudyOptions = function(){
	for(var i = 0; i < study_options.length; i++){
		study_options_li = $("<div id="+study_options[i]+"><input class='clearTarget' type='checkbox' value="+study_options[i]+" rel='Study Options' id="+study_options[i]+"><label for="+study_options[i]+">"+study_options[i]+"</label><br></div>")
...
var sortTable = function(){
	...
	var toggleBox = function(){
		...
		if ($("#states div:nth-child(n+5)").css({"display": "none"})){
			$("#states div:nth-child(n+5)").css({"display": "inline"})
			beenClicked = true
		} else if (beenClicked === true){
			$("#states div:nth-child(n+5)").css({"display": "none"})
		}
	}
*/
